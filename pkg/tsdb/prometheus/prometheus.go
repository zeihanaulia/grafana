...
func (s *Service) parseQuery(queryContext *backend.QueryDataRequest, dsInfo *DatasourceInfo) ([]*PrometheusQuery, error) {
	...
	for _, query := range queryContext.Queries {
		...
		if queryType == Range {
			...
			start = time.Unix(int64(math.Floor((float64(query.TimeRange.From.Unix())/interval.Seconds()))*interval.Seconds()), 0)
			end = time.Unix(int64(math.Floor((float64(query.TimeRange.To.Unix())/interval.Seconds()))*interval.Seconds()), 0)
		}
